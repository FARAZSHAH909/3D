...
public:
    ...
    Core()
        : adapter(nullptr), device(nullptr), graphicsQueue(nullptr), copyQueue(nullptr), computeQueue(nullptr), swapchain(nullptr), backbufferHeap(nullptr), backbuffers(nullptr), dsvHeap(nullptr), dsv(nullptr), rootSignature(nullptr)
    {
        graphicsCommandAllocator[0] = graphicsCommandAllocator[1] = nullptr;
        graphicsCommandList[0] = graphicsCommandList[1] = nullptr;
        graphicsQueueFence[0].fence = nullptr;
        graphicsQueueFence[1].fence = nullptr;
        // eventHandles and other init occur when fences are created
    }
	...
	void init(HWND hwnd, int _width, int _height)     // handle to window and width and height of window
	{
        ...
        // Try to create a D3D12 device on the best available hardware adapter.
        // Prefer non-software adapters and attempt multiple feature levels.
        HRESULT hr = E_FAIL;
        const D3D_FEATURE_LEVEL featureLevels[] = { D3D_FEATURE_LEVEL_12_1, D3D_FEATURE_LEVEL_12_0, D3D_FEATURE_LEVEL_11_0 };
        for (size_t ai = 0; ai < adapters.size(); ++ai)
        {
            DXGI_ADAPTER_DESC1 adesc;
            adapters[ai]->GetDesc1(&adesc);
            // skip software adapters (e.g. WARP / Microsoft Basic Render Driver)
            if (adesc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE)
                continue;

            // Try feature levels
            for (auto lvl : featureLevels)
            {
                ID3D12Device* baseDevice = nullptr;
                hr = D3D12CreateDevice(adapters[ai], lvl, IID_PPV_ARGS(&baseDevice));
                if (SUCCEEDED(hr) && baseDevice)
                {
                    // Query for the newer device interface used by the code
                    ...
                    {
                        // Couldn't get ID3D12Device5; try next feature level/adapter
                        device = nullptr;
                    }
                }
            }
        ...
        if (!device || FAILED(hr))
        {
            MessageBoxA(NULL, "Failed to create a Direct3D12 device. Check GPU drivers or try a different machine.", "D3D12 Device Error", MB_OK | MB_ICONERROR);
            // release factory and any adapter references
            for (auto a : adapters) if (a) a->Release();
		...
		hr = factory->CreateSwapChainForHwnd(graphicsQueue, hwnd, &scDesc, NULL, NULL, &swapChain1);
		if (FAILED(hr) || swapChain1 == nullptr) {
			MessageBoxA(NULL, "Failed to create swap chain.", "SwapChain Error", MB_OK | MB_ICONERROR);
			for (auto a : adapters) if (a) a->Release();
			factory->Release();
			return;
		}
		...
		if (FAILED(hr) || swapchain == nullptr) {
			MessageBoxA(NULL, "Failed to query swapchain interface.", "SwapChain Error", MB_OK | MB_ICONERROR);
			for (auto a : adapters) if (a) a->Release();
			factory->Release();
			return;
		}
